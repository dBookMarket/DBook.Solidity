...
describe(`platform dBook Platform`, () => {
  ...
  it(`platform contract => get platform address`, async () => {

    let chkPlatformAddress = await dDBookPlatformContract.getPlatformAddress();
    expect(chkPlatformAddress).to.equal(platform.address);

  })

  it(`platform contract => set platform address`, async () => {

    await dDBookPlatformContract.setPlatformAddress(seller.address);
    let chkPlatformAddress = await dDBookPlatformContract.getPlatformAddress();
    expect(chkPlatformAddress).to.equal(seller.address);
